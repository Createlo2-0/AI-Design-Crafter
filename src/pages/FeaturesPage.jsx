...
const IconAiChip = (props) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" d="M8.25 3v1.5M4.5 8.25H3m18 0h-1.5M4.5 12H3m18 0h-1.5m-15 3.75H3m18 0h-1.5M8.25 19.5V21M12 3v1.5m0 15V21m3.75-18v1.5m0 15V21m-9-7.5h12V12H6v2.25Z" />
    <path strokeLinecap="round" strokeLinejoin="round" d="M15 6.75h1.5V3H15v3.75Zm0 13.5h1.5v-3.75H15v3.75Zm-4.5-5.25v5.25H15v-5.25H10.5ZM7.5 9.75h1.5V3H7.5v6.75Zm0 10.5h1.5V15H7.5v5.25ZM4.5 6.75H6V3H4.5v3.75Zm0 10.5H6v-3.75H4.5v3.75ZM19.5 9.75h-1.5V3h1.5v6.75Zm0 10.5h-1.5V15h1.5v5.25Z" />
  </svg>
);
...
const FeatureCard = ({ icon, title, description, details, color, delay }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
    >
      {React.cloneElement(icon, { className: `w-10 h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 mb-4 text-${color}` })}
      <h3 className={`font-cyber text-xl sm:text-2xl md:text-3xl text-${color} mb-3 sm:mb-4 uppercase tracking-wide`}>{title}</h3>
      <p className="font-mono text-gray-300 mb-3 sm:mb-4 text-xs sm:text-sm md:text-base flex-grow">{description}</p>
...
function FeaturesPage() {
  const features = [
    {
      icon: <IconAiChip />,
      title: "AI Core X-1",
      description: "Harnessing quantum-entangled diffusion matrices, the AI Core X-1 translates abstract directives into hyper-realistic visual outputs. It comprehends nuanced semantics and complex compositional requests with unprecedented accuracy.",
      details: [
    ...
    },
    {
      ...
      description: "Interface with a vast library of pre-calibrated stylistic codecs or sculpt your own unique visual language. From neo-noir cityscapes to bio-luminescent flora, the matrix adapts.",
      details: [
    ...
    },
    {
      ...
      description: "Assume direct command over the generation parameters. Modulate CFG scale, step counts, sampler algorithms, and inject negative directives to refine outputs to exacting specifications.",
      details: [
    ...
    },
    {
      ...
      description: "All synthesized assets are logged within your encrypted datastore, accessible only via authenticated protocols. Manage, version, and export your creations with robust security.",
      details: [
  ...
  return (
    ...
    >
      ...
      >
        Accessing detailed operational parameters and capability matrix for DesignCrafter.AI Protocol. All systems nominal. Augment your reality.
      </motion.p>
      ...
      <motion.div
        ...
        variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { delay: 0.5 + features.length * 0.2 } } }}
      >
        <Link to="/generate">
          <motion.button
            className="bg-neon-green text-cyber-bg-darker font-bold py-3 px-8 sm:px-10 border-2 border-neon-green hover:bg-transparent hover:text-neon-green transition-all duration-300 ease-in-out hover:shadow-neon-lg-green rounded-sm text-base sm:text-lg"
